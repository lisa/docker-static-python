...
ADD \
  ...
  ./python-BPO-19348-3.6.6-fix-testcapi.patch \
 /build/
...
RUN \
  ...
  echo '_multiprocessing _multiprocessing/semaphore.c _multiprocessing/multiprocessing.c' >> Modules/Setup && \
  ...
  echo "Patching for https://bugs.python.org/issue19348" && \
  cd Modules && \
  patch --ignore-whitespace -p0 < /build/python-BPO-19348-3.6.6-fix-testcapi.patch && \
  cd .. && \
  sed -i \
  ...
  cd Lib && zip -r ../python${PYTHON_BASE_VERSION}.zip .
...
COPY \
  --from=0 \
    ...
    /build/Python-${PYTHON_BASE_VERSION}${PYTHON_RC}/python${PYTHON_BASE_VERSION}.zip \ 
  /
...
ENV \
  PYTHONHOME=/python${PYTHON_BASE_VERSION}.zip \
  PYTHONPATH=/python${PYTHON_BASE_VERSION}.zip
